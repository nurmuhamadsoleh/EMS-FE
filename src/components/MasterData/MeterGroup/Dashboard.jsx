...
import { FiEdit2, FiLayers, FiPlus, FiTrash } from "react-icons/fi";
...
const DataTable = () => {
  ...
  const getData = async () => {
    showLoader();
    await axios({
      url: `${url.api}/metergroup?search_query=${keyword}&page=${page}&limit=${limit}`,
      method: "GET",
      headers: tokenHeader(),
      data: {},
    })
  ...
  const handleOnExport = () => {
    var wb = XLSX.utils.book_new(),
      ws = XLSX.utils.json_to_sheet(mastergroup);
    // untuk sebuah array
              ...
              >
                <thead>
                  <tr className="text-center">
                    <th scope="col text-center">MasterGroupId</th>
                    <th scope="col text-center">MasterGroupName</th>
                    <th scope="col text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {mastergroup.map((item) => (
                    <tr className="table-active">
                      <td className="table-active">{item.metergroupid}</td>
                      <td className="table-active">{item.metergroupname}</td>
                        ...
                        <div className="row">
                          <div className="col-sm-6">
                            <Button
                              ...
                              size="sm"
                              className="me-1 px-3"
                              onClick={() => handleEdit(item.id_seq)}
                            >
                              <FiEdit2 /> Edit
                            </Button>
                          </div>
                          <div className="col-sm-6">
                            <Button
                              ...
                              size="sm"
                              onClick={() => onClickDelete(item.id_seq)}
                            >
                              <FiTrash /> Delete
                            </Button>
                          </div>
